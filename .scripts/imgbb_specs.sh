...
for i in *.flac ; do 
    ...
    full="$(curl -s -L -X POST "https://api.imgbb.com/1/upload?key=${imgbb_api_key}" -F "image=@${full_spectral}" | jq -r '.data.image.url')"
    ...
    zoomed="$(curl -s -L -X POST "https://api.imgbb.com/1/upload?key=${imgbb_api_key}" -F "image=@${zoom_spectral}" | jq -r '.data.image.url')"
